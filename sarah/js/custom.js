...
// sampleElm.innerHTML = `<p>Hello Woeld</p>`; // 2. 태그 삽입
...
imgWrap.addEventListener("mousemove", function (event) {
  let x = event.offsetX; // 마우스  x축 좌표값 지정
  const screenWitdth = document.body.offsetWidth; // 햔재 브라우저 가로 크기 지정
  const imgElm = document.querySelectorAll("section img"); // 이미지   태그 저장
  const percent = Math.floor((x / screenWitdth) * 200); // 전체 가로크기를 200으로 나눈 커서가 위치하는 x축 좌표 인덱스
